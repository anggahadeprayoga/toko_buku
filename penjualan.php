...
$req = "?menu=penjualan";
$tabel="tbl_buku";
$tabelsementara = "tbl_tmp_penjualan";
$tabelpenjualan = "tbl_penjualan";
@$id_fak = $_POST['id_fak'];
@$id_buku = $_GET['id'];
@$judul = $_POST['judul'];
...
@$stok = $_POST['stok'];
@$total = $_POST['total'];
@$fieldsementara = array('id_buku'=>$_GET['id'],'judul'=>$_POST['judul'],'jumlah_beli'=>$_POST['jumlah'],'total_harga'=>$_POST['total']);
@$where="id_buku = '$_GET[id]'";
$date= date('Y-m-d');

$go = new controller();

$query="SELECT * FROM tbl_penjualan ORDER BY id_penjualan desc LIMIT 1";
...
if(isset($_POST['simpanbuku'])){
  $sql="INSERT INTO tbl_tmp_penjualan VALUES ('$id_buku','$judul','$jumlah','$total')";
...
if(isset($_GET['update'])){
  $q="UPDATE tbl_tmp_penjualan SET jumlah_beli = '$jumlah', total_harga = '$total' WHERE id_buku = '$id_buku' )";
...
if(isset($_GET['hapus'])){
  $delete=mysqli_query($con,"DELETE FROM tbl_tmp_penjualan WHERE id_buku ='$id_buku' ");
  if($delete){
...
if(isset($_POST['simpantransaksi'])){
  @$sql = mysqli_query($con,"SELECT * FROM tbl_tmp_penjualan");
  while($data = mysqli_fetch_assoc($sql)){
    ...
    @$jalan = mysqli_query($con,"SELECT * FROM tbl_buku WHERE id_buku = '$id_buku'");
    $sisastok = mysqli_fetch_assoc($jalan);
    $sisa = $sisastok['stok'];
    $sisa = intval($sisa)-intval($total_beli);
    @$stok = mysqli_query($con,"UPDATE tbl_buku SET stok ='$sisa' WHERE id_buku = '$id_buku'");
    @$query=mysqli_query($con,"INSERT INTO tbl_penjualan VALUES('$id_fak','$id_buku','$id_user','$total_beli',' $bayar','$kembalian','$total_harga','$date')");
  }
  $delete = mysqli_query($con,"DELETE FROM tbl_tmp_penjualan");
  if($query){
...
<div class="card">
        ...
        <div>
            ...
            <?php 
            @$sql = mysqli_query($con,"SELECT * FROM tbl_tmp_penjualan WHERE judul = '$tampil[judul]'");
        ...
        <div class="text-secondary mt-5 border border-4  rounded py-4 px-2 shadow border-secondary border-start-0 border-end-0">
          <table class="mt-4 table table-stripped table-hover bg-white" id="data">
              <thead>
                <tr>
                  <th>Judul Buku</th>
                  <th>Jumlah Beli</th>
              ...
              <tbody>
                ...
                <tfoot>
                  <tr>
                    ...
                    <td>Total Harga</td>
                    <?php 
                      $sql = mysqli_fetch_array(mysqli_query($con,"SELECT total_harga,sum(total_harga) AS total_harga FROM tbl_tmp_penjualan"));
                    ?>
              ...
              <div class="modal-body">
                <div class="text-secondary mt-5 border border-4  rounded py-4 px-2 shadow border-secondary border-start-0 border-end-0">
                  <table  id="tabelbukum" class="data table table-striped table-bordered border-light table-hover py-3 display">
                    <thead>
                        <tr class="table-light rounded-shadow">
                            <th>Id Buku</th>
                            <th>Judul</th>
                            <th>stok</th>
                            <th>Harga Jual</th>
                    ...
                    <tbody class="table-light">
                        <?php 
                            ...
                            $no = 0;
                            $sql = "SELECT * from tbl_buku WHERE stok != 0";
